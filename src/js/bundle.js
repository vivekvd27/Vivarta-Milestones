...
function initializeApp() {
  ...
  if (!window.supabaseReady) {
    ...
    if (window.appState) {
      ...
      console.log("   📊 window.appState.teamTasks CONTENT before sync:", window.appState.teamTasks);
      ...
      if (appState.ruleOfThree && appState.ruleOfThree.length > 0) {
        console.log("   🎯 Rule Of Three tasks loaded (first 3):", appState.ruleOfThree.slice(0, 3).map(t => `${t.person}: ${t.task}`).join(", "));
      }
      console.log("   teamTasks keys:", Object.keys(appState.teamTasks || {}));
      console.log("   📊 appState.teamTasks CONTENT after sync:", appState.teamTasks);
      
      // Show detailed info about each person's tasks
      Object.entries(appState.teamTasks || {}).forEach(([person, tasks]) => {
        const taskTexts = tasks?.map(t => t?.text || '[no text]').join(', ') || 'none';
        console.log(`      ${person}: ${tasks?.length || 0} tasks = ${taskTexts}`);
      });
    }
  }
